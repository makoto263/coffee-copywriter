...
### 1) Google Colab / 학습 및 추론 절차
1. 본 리포지토리에 포함된 `.ipynb` 파일을 Google Colab에 업로드.
2. Unsloth 및 Transformers 라이브러리 설치 셀을 실행.
3. 데이터셋 로드 및 파인튜닝 코드를 순차적으로 실행하여 GGUF 파일로 익스포트.

### 2) 로컬 환경 (Ollama) 실행 방법
1. 에스포트된 `gemma2-2b-it.Q4_K_M.gguf` 파일과 본 리포지토리의 `Modelfile`을 동일한 디렉토리에 위치시킨다.
2. 터미널을 열고 해당 폴더로 이동 후 아래 명령어를 실행하여 모델을 빌드한다:
```bash
